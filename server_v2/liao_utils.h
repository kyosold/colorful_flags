...
//int new_idx_from_child_st(struct childs *child_st);
...
//int send_socket_cmd(int parent_w_fd, char *pid, char *fuid, char *fnick, char *type, char *tuid);
...
int has_offline_msg(struct config_t *config_st, char *myuid);
...
//int write_b64_content_to_file_with_uid_mfile(struct config_t *config_st, char *tuid, char *content, size_t content_len, MFILE *mfp_in, char *old_img_name, char *file_name, size_t file_name_size);
