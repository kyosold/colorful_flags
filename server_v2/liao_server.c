...
{
	/*if (child_st[i].fd_in != -1) {
		close(child_st[i].fd_in);
		child_st[i].fd_in = -1;
	}
	if (child_st[i].fd_out != -1) {
		close(child_st[i].fd_out);
		child_st[i].fd_out = -1;
	}*/
			...
			*psed = ' ';
			...
			if (pbuf != NULL) {
                if (strncasecmp(pbuf, "tios_token:", 11) == 0) { 
                    r = snprintf(cmd_tios_token, sizeof(cmd_tios_token), "%s", pbuf+11);
                    while ( cmd_tios_token[r-1] == '\r' || cmd_tios_token[r-1] == '\n' )
                        r--; 
                    cmd_tios_token[r] = '\0';
                    log_debug("from tios_token:%d:%s", r, cmd_tios_token);
                }    
            } 
			...
			if (evt_fd == listenfd) {
				if ((connfd = accept(listenfd, (struct sockaddr *) &remote, &addrlen)) > 0) {
					...
					if (f == 0) {
						...
						//if (fd_move(0, piw[0]) == -1) {
							log_info("%s fd_move(0, %d) failed:%d %s", child_st[i].mid, connfd, errno, strerror(errno));
							_exit(111);
						}

						/*if (fd_move(1, pi[1]) == -1) {
							log_info("%s fd_move(1, %d) failed:%d %s", child_st[i].mid, pi[1], errno, strerror(errno));
							_exit(111);
						}

						if (fd_copy(2, 1) == -1) {
							log_info("%s fd_move(2, 1) failed:%d %s", child_st[i].mid, errno, strerror(errno));
							_exit(111);
						}*/
					...
					/*if (setnonblocking(child_st[i].pipe_w_out) != 0) {
						log_error("setnonblocking fd[%d] failed", child_st[i].pipe_w_out);
					}*/
					
					struct epoll_event pipe_in_ev;
					pipe_in_ev.events = EPOLLIN | EPOLLET;	
					//ev.data.fd = child_st[i].fd_in;
					...
					/*struct epoll_event pipe_w_ev;
					pipe_w_ev.events = EPOLLIN | EPOLLET;
					pipe_w_ev.data.fd = child_st[i].pipe_w_out;
					if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, pipe_w_ev.data.fd, &pipe_w_ev) == -1) {
						log_error("epoll_ctl client fd[%d] EPOLL_CTL_ADD failed:[%d]%s", pipe_w_ev.data.fd, errno, strerror(errno));
					}
					log_debug("epoll_add fd[%d]", pipe_w_ev.data.fd);*/
					...
					//log_debug("write to child fd:%d", child_st[i].pipe_w_out);
					//char sjtest[] = "okokokokokokok\n";
					//write(child_st[i].pipe_w_out, sjtest, strlen(sjtest));
				...
				//log_debug("%s get event EPOLLIN: epoll_i[%d] fd[%d] get d_i[%d] fd[%d], used[%d]", child_st[ci].mid, epoll_i, epoll_evts[epoll_i].data.fd, child_st[ci].fd_in, child_st[ci].used);
				...
				/*while (1) {
					memset(inbuf, 0, inbuf_size);
					int r = mread(child_st[ci].mfp_out, inbuf, inbuf_size);
					log_debug("get data from command:%d:%s", r, inbuf);
					if (r == 0)	break;
					if (r < 0) log_error("mread fail");
				}*/
				...
				//log_debug("%s get event EPOLLHUP: epoll_i[%d] fd[%d] get d_i[%d] fd[%d], used[%d]", child_st[ci].mid, epoll_i, epoll_evts[epoll_i].data.fd, child_st[ci].fd_in, child_st[ci].used);
				...
				//epoll_delete_evt(epoll_fd, child_st[ci].fd_in);
