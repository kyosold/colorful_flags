...
int is_GZippedData(Byte *compr, uLong compr_len);
...
int login_cmd(struct session *session, char *pbuf, int pbuf_size, char *fail_desc, size_t fail_desc_size)
{
		...
		if (psed != NULL) {
			*psed = '\0';
			...
			if (m > 0) {
				...
				if (strncasecmp(pbuf, "account:", 8) == 0) {
					r = snprintf(account, sizeof(account), "%s", pbuf+8);
					log_debug("from account:%d:%s", r, account);	

				} else if (strncasecmp(pbuf, "password:", 9) == 0) {
					r = snprintf(password, sizeof(password), "%s", pbuf+9);
					log_debug("from password:%d:%s", r, password);

				} else if (strncasecmp(pbuf, "ios_token:", 10) == 0) {
					r = snprintf(self_ios_token, sizeof(self_ios_token), "%s", pbuf+10);
					log_debug("from ios_token:%d:%s", r, self_ios_token);

				} else if (strncasecmp(pbuf, "get_friend_list:", 16) == 0) {
					get_friend_list = atoi(pbuf+16);
					log_debug("from get_friend_list:%d", get_friend_list);
			...
			if (pbuf != NULL) {
				if (strncasecmp(pbuf, "get_friend_list:", 16) == 0) {
					get_friend_list = atoi(pbuf+16);
					log_debug("from get_friend_list:%d", get_friend_list);
				}
			}
	...
	result_size = 0;
	presult = login_http_api(url, account, password, get_friend_list, &result_size);
	...
	struct json_object *js_status;
	struct json_object *js_desc;
	...
	json_ret = json_object_object_get_ex(result_json, "status", &js_status);
	if ( json_ret == 0) {
		log_error("get status fail with json_object_object_get_ex key:status");
		goto FINISH;
	}
	
	char *ret_status = (char *)json_object_to_json_string(js_status);
	if (ret_status) {
		if ( strcasecmp(ret_status, "fail") == 0) {
			json_ret = json_object_object_get_ex(result_json, "des", &js_desc);
			if (json_ret) {
				char *ret_desc = (char *)json_object_to_json_string(js_desc);
				snprintf(fail_desc, fail_desc_size, "%s", ret_desc);
			}
			goto FINISH;
		}	
	}
	...
	// 注册登录
	int ret;
	if (*self_ios_token != '\0' && strlen(self_ios_token) > 0) {
		ret = user_login(&config_st, 0, uid, self_ios_token);
		if (ret == -1) {
			log_error("register login fail");
			return 1;	
		}
	}
	...
	snprintf(session->ios_token, sizeof(session->ios_token), "%s", self_ios_token);

	pid_t cpid = getpid();
	char mc_value[MAX_LINE] = {0};
	//snprintf(mc_value, sizeof(mc_value), "%d,%s", cpid, ios_token);
	...
	if (ret != 0) {
		log_error("add to online failed uid[%s] sockfd[%d] ios_token[%d]", uid, sockfd, cpid);	
		
		if (*self_ios_token != '\0' && strlen(self_ios_token) > 0) {
			user_login(&config_st, 1, uid, self_ios_token);
		}
	...
	log_debug("registon online succ uid[%s] ios_token[%s] sockfd[%d]", uid, self_ios_token, sockfd);	

	if (session->client_type == 1) {
		send_to_websocket(session->client_fd, response_buf, n);
	} else {
		send_to_socket(session->client_fd, response_buf, n);
	}
...
// logout fuid:0000001 
int logout_cmd(struct session *session, char *pbuf, int pbuf_size)
{
	log_debug("pbuf:%d:%s", pbuf_size, pbuf);
	...
	while ((n > 0) && (pbuf != NULL) && (*pbuf != '\0')) {
		...
		if (psed != NULL) {
			*psed = '\0';
			...
			if (pbuf != NULL) {
				if (strncasecmp(pbuf, "fios_token:", 11) == 0) {
					r = snprintf(fios_token, sizeof(fios_token), "%s", pbuf+11);
					while ( fios_token[r-1] == '\r' || fios_token[r-1] == '\n' )
                        r--;
                    fios_token[r] = '\0';
					log_debug("from fios_token:%d:%s", r, fios_token);
	...
	// 设置离线
	// 查询fd
	if (strcasecmp(session->uid, fuid) != 0) {
		log_error("logic error session.uid:%s != fuid:%s", session->uid, fuid);
		return 1;
	}

	// 注册登出
	user_login(&config_st, 1, fuid, session->ios_token);

	// 注册为离线	(uid => ios_token)
	int sockfd = session->client_fd;
	int ret = delete_from_mc(config_st.mc_server, config_st.mc_port, atoi(config_st.mc_timeout), fuid);
	if (ret != 0) {
		log_error("delete from online failed uid[%s] sockfd[%d] ios_token[%s]", fuid, sockfd, session->ios_token);	
		return 1;

	}
	log_debug("offline fuid:%s", fuid);
...
int helo_cmd(struct session *session, char *pbuf, int pbuf_size, char *myuid, size_t myuid_size)
{
		...
		if (psed != NULL) {
			*psed = '\0';
			...
			if (strncasecmp(pbuf, "fuid:", 5) == 0) {
				...
				r = snprintf(self_ios_token, sizeof(self_ios_token), "%s", pbuf+11);
				log_debug("from ios_token:%d:%s", r, self_ios_token);
			...
			if (pbuf != NULL) {
				if (strncasecmp(pbuf, "fios_token:", 11) == 0) {
					r = snprintf(self_ios_token, sizeof(self_ios_token), "%s", pbuf+11);
					while ( self_ios_token[r-1] == '\r' || self_ios_token[r-1] == '\n' )
						r--;
					self_ios_token[r] = '\0';
					log_debug("from ios_token:%d:%s", r, self_ios_token);	
				}
	...
	log_debug("get sockfd[%d] from fios_token[%s]", sockfd, self_ios_token);
	...
	int ret;
	if (*self_ios_token != '\0' && strlen(self_ios_token) > 0) {
		ret = user_login(&config_st, 0, fuid, self_ios_token);
		if (ret == -1) {
			log_error("register login fail");
			return 1;	
		}
	...
	// 注册为在线	(uid => ios_token)
    snprintf(session->uid, sizeof(session->uid), "%s", fuid);
    snprintf(session->ios_token, sizeof(session->ios_token), "%s", self_ios_token);

	pid_t cpid = getpid();
    char mc_value[MAX_LINE] = {0}; 
    //snprintf(mc_value, sizeof(mc_value), "%d,%s", cpid, fios_token);
    snprintf(mc_value, sizeof(mc_value), "%d", cpid);

	ret = set_to_mc(config_st.mc_server, config_st.mc_port, atoi(config_st.mc_timeout), fuid, mc_value);
	if (ret != 0) {
		log_error("add to online failed uid[%s] sockfd[%d] ios_token[%s]", fuid, sockfd, session->ios_token);	
		return 1;
	} else {
		log_debug("registon online succ uid[%s] ios_token[%s] sockfd[%d]", fuid, session->ios_token, sockfd);	
...
int sendreq_addfriend(struct session *session, char *pbuf, int pbuf_size, char *mid, size_t mid_size)
{
	log_debug("pbuf:%d:%s", pbuf_size, pbuf);
	...
	while ((n > 0) && (pbuf != NULL) && (*pbuf != '\0')) {
		...
		if (psed != NULL) {
			*psed = '\0';
			...
			if (pbuf != NULL) {
				if (strncasecmp(pbuf, "tios_token:", 11) == 0) {
					r = snprintf(tios_token, sizeof(tios_token), "%s", pbuf+11);
					while ( tios_token[r-1] == '\r' || tios_token[r-1] == '\n' )
                        r--;
                    tios_token[r] = '\0';
					log_debug("from tios_token:%d:%s", r, tios_token);
				}
	...
	// 写消息索引到数据库
	n = write_queue_to_db(&config_st, TAG_RECVADDFRIENDREQ, mid, fuid, fnick, fios_token, tuid, tios_token, m_type, "", 0);
	if (n == -1) {
		goto SNDFRD_FAIL;
	}
...
void command(struct session *session)
{
	...
	// LOGIN: 用于表示客户端登录 LOGIN account:abalam password:123qwe ios_token:111 get_friend_list:1 \r\n\r\n
	if ( strncasecmp(fbuf, "LOGIN ", 6) == 0 ) {
		char *pbuf = fbuf + 6;
		n -= 6;
		char fail_desc[MAX_LINE] = {0};
		int ret = login_cmd(session, pbuf, n, fail_desc, sizeof(fail_desc));	
		if (ret != 0) {
			// 认证失败
			n = snprintf(rspsoneBuf, sizeof(rspsoneBuf), "%s FAIL %s%s", TAG_LOGIN, fail_desc, DATA_END);
			log_debug("send to client:%d:%s", n, rspsoneBuf);
	...
	// LOGOUT: 用户登录
	if ( strncasecmp(fbuf, "LOGOUT ", 7) == 0 ) {
		char *pbuf = fbuf + 7;
		n -= 7;
		int ret = logout_cmd(session, pbuf, n);
	}
	...
	if ( strncasecmp(fbuf, "helo ", 5) == 0 ) {
		...
		if (ret == 0) {
			// 检查是否有离线消息
			int offline_msg_num = has_offline_msg(&config_st, myuid);

			n = snprintf(rspsoneBuf, sizeof(rspsoneBuf), "%s OK %d%s", TAG_HELO, offline_msg_num, DATA_END);
		} else {
	...
	if ( strncasecmp(fbuf, "SENDADDFRDREQ ", 14) == 0 ) {
		...
		n -= 14;
		char mid[MAX_LINE] = {0};
		int ret = sendreq_addfriend(session, pbuf, n, mid, sizeof(mid));
		if (ret == 0) {
			n = snprintf(rspsoneBuf, sizeof(rspsoneBuf), "%s OK %s%s", TAG_SENDADDFRIENDREQ, mid, DATA_END);
		} else {
			n = snprintf(rspsoneBuf, sizeof(rspsoneBuf), "%s FAIL %s%s", TAG_SENDADDFRIENDREQ, mid, DATA_END);
		}
...
void system_command(MFILE *mfp_parent)
{
	...
	if (n > 0) {
		...
		// type:SSYSTEM_SENDMSG pid:16053 fuid:47 fnick:czAwMDI= mtype:SYS tuid:44 tios_token:e81bf81e5c74724cd44f0fe72524a52f27f8d567030c91053671256e2edbd9b7	
		if ( strncasecmp(fbuf, "type:SSYSTEM_SENDMSG ", 21) == 0 ) {
			// 内部通信
			char cmd_pid[128] = {0}; 
			char cmd_fuid[128] = {0}; 
			char cmd_fnick[MAX_LINE] = {0}; 
			char cmd_mtype[128] = {0}; 
			char cmd_tuid[128] = {0}; 
			//char cmd_tios_token[MAX_LINE] = {0};
					...
					*psed = '\0';
					...
					if (m > 0) { 
						...
						if (strncasecmp(pbuf, "pid:", 4) == 0) {
							...
							//r = snprintf(cmd_tios_token, sizeof(cmd_tios_token), "%s", pbuf+11);
							//log_debug("from cmd_tios_token:%d:%s", r, cmd_tios_token);
...
int is_GZippedData(Byte *compr, uLong compr_len)
{
	const unsigned char *bytes = (const unsigned char *)compr;
	log_info("length:%d bytes[0]:%d bytes[1]:%d", compr_len, bytes[0], bytes[1]);
	return (compr_len >= 2 && bytes[0] == 0x1f && bytes[1] == 0x8b);
}
